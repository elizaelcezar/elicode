...
# Usuario do servico: quem chamou via sudo, o usuario atual, ou root.
if [ -n "${SUDO_USER:-}" ]; then SERVICE_USER="$SUDO_USER";
elif [ "$(id -un)" != "root" ]; then SERVICE_USER="$(id -un)";
...
echo "  2. Copie seu ~/.config/opencode/opencode.json (auth dos modelos) para $USER_HOME/.config/opencode/"
